...
from grow_with_the_flow.hooks.cloud_base_hook import GoogleCloudBaseHook
...
class GCSHook(GoogleCloudBaseHook):
    ...
    def get_conn(self):
        if not self._conn:
            try:
                ...
                # credential_file = os.path.join(credential_path, 'iot-production-7efe688928c0.json')
                self._conn = storage.Client.from_service_account_json(
                    f"{credential_path}/iot-production-7efe688928c0.json"
                )
    ...
    def rewrite(
        ...
        self.log.info(
            "Total Bytes: %s | Bytes Written: %s", total_bytes, bytes_rewritten
        )
    ...
    def upload(
        ...
        if filename and data:
            ...
            self.log.info(
                "Data stream uploaded to %s in %s bucket", object_name, bucket_name
            )
        else:
            raise ValueError(
                "'filename' and 'data' parameter missing. "
    ...
    def is_updated_after(self, bucket_name, object_name, ts):
        ...
        if blob is None:
            raise ValueError(
                "Object ({}) not found in Bucket ({})".format(object_name, bucket_name)
            )
    ...
    def list(
        self, bucket_name, versions=None, max_results=None, prefix=None, delimiter=None
    ):
        """
    ...
    def get_md5hash(self, bucket_name, object_name):
        ...
        """
        self.log.info(
            "Retrieving the MD5 hash of " "object: %s in bucket: %s",
            object_name,
            bucket_name,
        )
        client = self.get_conn()
    ...
    def _prepare_sync_plan(
        ...
        # about the parent's path
        source_object_prefix_len = len(source_object) if source_object else 0
        destination_object_prefix_len = (
            len(destination_object) if destination_object else 0
        )
        delimiter = "/" if not recursive else None
        # Fetch blobs list
        source_blobs = list(
            source_bucket.list_blobs(prefix=source_object, delimiter=delimiter)
        )
        destination_blobs = list(
            destination_bucket.list_blobs(
                prefix=destination_object, delimiter=delimiter
            )
        )
        # Create indexes that allow you to identify blobs based on their name
        source_names_index = {
            a.name[source_object_prefix_len:]: a for a in source_blobs
        }
        destination_names_index = {
            a.name[destination_object_prefix_len:]: a for a in destination_blobs
        }
        ...
        to_copy_blobs = {
            source_names_index[a] for a in to_copy
        }  # type: Set[storage.Blob]
        to_delete_blobs = {
            destination_names_index[a] for a in to_delete
        }  # type: Set[storage.Blob]
