...
class PostgresHook(BaseHook):
    ...
    def copy_string_iterator(
        self, connection, beers: Iterator[Dict[str, Any]], size: int = 8192
    ) -> None:
        with self.postgres_conn.cursor() as cursor:
            ...
            cursor.copy_from(data_string_iterator, "staging_beers", sep="|", size=size)

    def execute_insert_postgres_data(
        self, data: Iterator[Dict[str, Any]], page_size: int
    ) -> None:
        with self.postgres_conn.cursor() as cursor:
            ...
            self.execute_batch(
                cursor,
                f"""
                INSERT INTO {self.staging_table} VALUES (
                    # %(id)s,
                    # %(created_at)s,
                    # %(reader_count)s,
                    # %(label)s
